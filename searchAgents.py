...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    position, cornerPos = state
    distances = []

    if len(cornerPos) == 0:
        return 0
    
    # append all distances then return max
    for corPos in cornerPos:
        distances.append(util.manhattanDistance(position, corPos))
...
def foodHeuristic(state, problem):
    ...
    maxHeuristic = 0
    for food in foodPos:
        # get maze distance using function
        distance = mazeDistance(position, food, problem.startingGameState)    
        # get max distance out of all distances
        if maxHeuristic < distance:
            maxHeuristic = distance
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # check coordinates if they match food coordinates
